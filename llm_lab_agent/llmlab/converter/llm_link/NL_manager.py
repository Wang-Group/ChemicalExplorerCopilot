...
class NLManager():
                    ...
                    **(
                        {
                            ...
                            "concentration": value_temp["concentration"],
                            "solvent": value_temp["solvent"]
                         }
                        )
                    )
    ...
    def convert_property_to_standard_unit(self, raise_error_directly: bool = False):
        """
    ...
    def prepare_one_liquid_from_dilution(self, liquid: llmlab.utlis.property.Liquid):
        ...
        # check if we can get it directly first
            ...
            if bool(set(set1) & set(set2[index])):
                if (liquid.concentration.unit == self.storage_liquid[index].concentration.unit):
                    ...
                    if (liquid.concentration.quantity == self.storage_liquid[index].concentration.quantity):
                        if "pump" in list(self.storage_liquid[index].device.keys()):
                            return "Liquid",  self.storage_liquid[index]
                        elif "reactor" in list(self.storage_liquid[index].device.keys()):
                            return "Liquid_from_reactor", self.storage_liquid[index]
                        else:
                            raise ValueError("the connection is not supported in this platform!")
                    
        # then check if we can get it from dilution         
        for index in range(len(set2)):
            # find the liquid
            if bool(set(set1) & set(set2[index])):
                if (liquid.concentration.unit == self.storage_liquid[index].concentration.unit):
                    if self.storage_liquid[index].concentration.quantity > liquid.concentration.quantity:
                        # dirty fix for now
                        if "pump" in list(self.storage_liquid[index].device.keys()):
                            return "Dilute",self.storage_liquid[index]
                        elif "reactor" in list(self.storage_liquid[index].device.keys()):
                            return "Dilute_from_reactor", self.storage_liquid[index]
                        else:
                            raise ValueError("the connection is not supported in this platform!")
    ...
    def check_material_availability_q(self, ignore_quantity = True):
        # for each step, convert the units from molar to g/mL
    ...
    def check_function_ambiguity(self, verbose = True):
        ...
        while True:
            ...
            for step_count in range(len(self.parsed_functions)):
                ...
                # reset the stir flag according to the stirring speed from the user
                if hasattr(step, "stir"):
                    if step.stir == True:
                        if step.stir_speed.quantity == 0:
                            step.stir = False
    ...
    def get_consumption(self, reagents_usage, solid_prepare_procedure, liquid_prepare_procedure, tolerance = 0.2, abs_tolerance = 5, resolutin = 10):        
        ...
        for index in range(len(self.required_liquid)):
            ...
            if (liquid_prepare_procedure[index][0] == "Liquid") or (liquid_prepare_procedure[index][0] == "Liquid_from_reactor"):
            ...
            elif (liquid_prepare_procedure[index][0] == "Dilute") or (liquid_prepare_procedure[index][0] == "Dilute_from_reactor"):
