    ...
    def sim_execute(self, graph: exp_graph):
        ...
        for i in range(self.repeat):
            graph.reactors[self.waste_vessel].content.append(
                (deepcopy(self.solvent), deepcopy(self.volume))
            )
...
class Dry(BasicStep):
    ...
    def check_ambiguity(self, mode = "plain_txt"):
        ...
        if not isinstance(self.time, Property):
            self.ambiguity_dict["time"] = copy(self.time)
...
class Precipitate(BasicStep):
        ...
        if self.reagent is not None:
            for i in range(self.repeat):
                graph.reactors[self.reactor_name].content.append(
                    (deepcopy(self.reagent), deepcopy(self.reagent_quantity))
                )
    ...
    def check_ambiguity(self, mode = "plain_txt"):
        ...
        # if we need to add a reagent
        if self.reagent != None:
        ...
        # the stirring time should be a specific value 
        if not isinstance(self.stirring_time, Property):
            self.ambiguity_dict["stirring_time"] = copy(self.stirring_time)
