...
class TransferLiquid(BasicStep):
    ...
    def __init__(self, 
                 from_reactor: str, 
                 to_reactor: str,
                 volume: Union[dict[float, str],str, None],
                 stir: bool = False,
                 stir_speed: str = None,
                 dropwise: bool = False):
    ...
    def sim_execute(self, graph: exp_graph):
        ...
        if self.volume == "all":
            ...
            content_temp = deepcopy(graph.reactors[self.from_reactor].content)
        ...
        # transfer only part of the liquid 
            ...
            if self.volume.unit == "mL":
                ...
                for chemical in graph.reactors[self.from_reactor].content:
                    ...
                    chemical[1].quantity = deepcopy(chemical[1].quantity * (1-percentage))
            else:
        ...
        # transfer only part of the liquid 
            ...
            if self.mass.unit == "g":
                ...
                for chemical in graph.reactors[self.from_reactor].content:
                    ...
                    graph.reactors[self.to_reactor].content.append(temp)
                    # decrease the amount in the from_reactor
                    chemical[1].quantity = deepcopy(temp[1].quantity * (1-percentage))
            else:
