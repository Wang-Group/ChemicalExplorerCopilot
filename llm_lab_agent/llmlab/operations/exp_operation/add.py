...
class AddLiquid(BasicStep):
    ...
    def check_ambiguity(self):
        ...
        if self.stir_speed == None:
            if self.stir == True:
                self.ambiguity_dict["stir_speed"] = "unknown" # to be clarified by the user
        else:
            if not isinstance(self.stir_speed, Property): 
                self.ambiguity_dict["stir_speed"] = copy(self.stir_speed) # to be clarified by the user
    ...
    def sim_execute(self, graph: exp_graph):
        ...
        graph.reactors[self.reactor_name].content.append(
            (deepcopy(self.liquid), deepcopy(self.volume))
        )
...
class AddSolid(BasicStep):
    ...
    def sim_execute(self, graph: exp_graph):
        ...
        """
        graph.reactors[self.reactor_name].content.append(
            (deepcopy(self.solid), deepcopy(self.mass))
        )
    def convert_molar_units(self):
        """
        Convert molar to g according to the molecular weight
        """
        if self.mass.unit =="mol":
            self.mass.quantity = self.mass.quantity*self.solid.MW
            self.mass.unit = "g"
