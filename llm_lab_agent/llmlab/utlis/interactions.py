...
def search_chemical_id(compound: Union[Solid, Liquid]):
    for chemical_id_index in range(len(compound.identity.chemical_id)):
        for identical_name in identical_names:
            if compound.identity.chemical_id[chemical_id_index] in identical_name[0]:
                compound.identity.chemical_id.append(identical_name[0][0])
                compound.identity.chemical_id = list(dict.fromkeys(compound.identity.chemical_id[::-1]))
                return identical_name[1]
    return None
...
def search_name_MW(compound: Union[Solid, Liquid]):
    ...
    while True:
        ...
        # if it exists in our original dataset with MW, return it directly
        temp_MW = search_chemical_id(compound = compound)
        if temp_MW is not None:
            return (compound.identity.chemical_id[0], "Unknown"), temp_MW
        
        # else, use the name of the chemical to search. 
...
def ask_for_physical_property(property_name, property_value, mode = "plain_txt"):
    ...
    if mode == "plain_txt":
        ...
        print(f"""Use -u if the input has a unit (-u 20 degrees), -f if the string is a number (-f 5.2), or -s if the input is a string (-s one string).""", flush = True)
