...
class DobotApi:
    def __init__(self, ip, port, logger: logging.Logger = None, *args):
        self.ip = ip
        self.port = port
        self.socket_dobot = 0
    ...
    def send_data(self, string):
        self.log(f"Send to 192.168.192.6:{self.port}: {string}")
    ...
    def wait_reply(self):
        ...
        self.log(f'Receive from 192.168.192.6:{self.port}: {data_str}')
        return data_str
    ...
    def MovJIO(self, x, y, z, r, *dynParams):
        ...
        *dynParams :Parameter Settings（Mode、Distance、Index、Status）
        ...
        string = "MovJIO({:f},{:f},{:f},{:f}".format(
            x, y, z, r)
        self.log("Send to 192.168.192.6:29999:" + string)
        for params in dynParams:
            string =string+ ","+ str(params)
        string =string+ ")" 
        # print(string)
