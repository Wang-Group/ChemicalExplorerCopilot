...
def send_to_gpt(
    count, NL, model, CHAT_HISTORY, FUNCTION_INFO_GLOBAL, total_errors, sent=True
):
    """
    ...
    if count == 0:
        to_send = initial_prompt.format(
            function_set=FUNCTION_SET, reference_samples=None
        )
        if sent == False:
...
def one_round_check(gpt_response, FUNCTION_INFO_GLOBAL):
    """
    ...
    try:
        ...
        Json_Path = save_to_json(json_dict, working_path="./temp")
    except:
...
def prepare_one_batch_messages(content: pd.DataFrame, model: str = "gpt-4o"):
    """
...
def validate_finished_batch(content: pd.DataFrame, result_path):
    ...
    for res in results:
        ...
        # add one to teh current evaluation
        ...
        try:
            # validate the response
            json_mng = one_round_check(
                gpt_response, content_temp["function_info"]
            ...
            write_json(json_dict = json_to_write, json_path = f"./{index}.json")
